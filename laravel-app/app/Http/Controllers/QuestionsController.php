...
class QuestionsController extends Controller
{
    ...
    {
        return redirect()->route('landing')->with('success', 'Your submitted answers are saved. You can resume your assessment anytime.');
    }

    public function resume(AssessmentResumeService $resume)
    {
        $userId = (int) session('user_id');
        $dateOfBirth = session('user_dob') ?: WPUsers::where('user_id', $userId)->value('date_of_birth');
        $route = $resume->restore($userId, $dateOfBirth);

        return $route
            ? redirect($route)
            : redirect()->route('landing')->with('fail', 'There is no unfinished assessment to resume.');
    }
...
public function reset_questions()
{
    ...
    $wp_user->skill_brain_profile_id = null;
    $wp_user->update();
