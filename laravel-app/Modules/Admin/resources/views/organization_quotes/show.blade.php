...
@section('content')
  ...
  </div></div></section><aside><section class="panel payment-card"><h2 class="panel-title">Payment</h2><div class="notice" style="margin-top:18px">@if($quote->status === 'paid')Payment received. Assessment seats are active.@elseConfirm payment received to activate seats and email a paid invoice to {{ $quote->organization->contact_email }}.@endif</div>@if($quote->status !== 'paid')<form method="post" action="{{ url('admin/organization-quotes/'.$quote->id.'/mark-paid') }}">@csrf<label class="toggle-row"><input class="toggle" type="checkbox" onchange="if(this.checked){if(confirm('Confirm payment has been received?')){this.form.submit()}else{this.checked=false}}"> Payment received</label></form>@else<label class="toggle-row"><input class="toggle" type="checkbox" checked disabled> Payment received</label>
    @if($quote->invoice_sent_at)<p class="muted">Invoice emailed to {{ $quote->invoice_data['email'] }} on {{ $quote->invoice_sent_at->format('j M Y, g:i A') }}.</p>
    @else<p class="muted">{{ $quote->invoice_data ? 'Invoice email pending. You can retry sending it.' : 'No invoice email has been sent for this agreement.' }}</p><form method="post" action="{{ url('admin/organization-quotes/'.$quote->id.'/invoice') }}">@csrf<button class="primary-button">{{ $quote->invoice_data ? 'Retry invoice email' : 'Send invoice email' }}</button></form>@endif
    @endif</section>
