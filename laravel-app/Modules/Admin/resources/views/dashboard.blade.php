...
<div class="cards"><section class="panel"><div class="panel-head"><div><h2 class="panel-title">New corporate inquiries</h2><p class="panel-sub">Needs a first response</p></div><a class="view-link" href="{{ url('admin/organization-enquiries') }}">View all</a></div>@forelse($inquiries as $enquiry)<a class="list-row inquiry-link" href="{{ url('admin/organization-enquiries/'.$enquiry->id.'/create-deal') }}"><div><div class="row-name">{{ $enquiry->organization_name }}</div><div class="row-detail">{{ number_format($enquiry->group_size) }} people · {{ now()->parse($enquiry->created_at)->timezone(config('app.timezone'))->diffForHumans() }}</div></div><span class="badge {{ $enquiry->status === 'new' ? 'badge-new' : 'badge-talk' }}">{{ $enquiry->status === 'new' ? 'New' : 'In talks' }}</span></a>@empty<div class="empty">No new corporate inquiries.</div>@endforelse</section>
...
@push('scripts')
<style>
  .inquiry-link { color: inherit; text-decoration: none; }
  .inquiry-link:hover { background: var(--cream); }
  .inquiry-link:hover .row-name { text-decoration: underline; }
  .inquiry-link:focus-visible { outline: 3px solid var(--ink); outline-offset: -3px; background: var(--cream); }
</style>
@endpush
